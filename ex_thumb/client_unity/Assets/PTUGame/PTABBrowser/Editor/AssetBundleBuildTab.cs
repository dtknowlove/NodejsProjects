...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        private const string PATH_CONFIGFILE = "PTUGame/customconfig/abbrowserconfig.bin";
        internal void OnDisable()
        {

            var dataPath = Path.Combine(Application.dataPath, PATH_CONFIGFILE);
        ...
        internal void OnEnable(EditorWindow parent)
        {
            ...
            m_GenerateClass = new ToggleData(
                false,
                "Generate Class",
                "After build completes,will generate class to ",
                m_UserData.m_OnToggles
            );            
...
//        private bool isCustomOutputPath = false;
        internal void OnGUI()
        {
...
//            GUILayout.Label(new GUIContent("Example build setup"), centeredStyle);
            ...
            var newVersion = EditorGUILayout.TextField("Res Version", m_UserData.m_resVersion);

            if (newVersion != m_UserData.m_resVersion )
            {
                m_UserData.m_resVersion = newVersion;
            }
...
//                        m_UserData.m_UseDefaultPath = false;
                ...
                else
                {
                    ...
                    EditorGUILayout.LabelField("Output Path",m_UserData.m_OutputPath);
                }

                float originalLabelWidth = EditorGUIUtility.labelWidth;
                EditorGUIUtility.labelWidth = 50;
//                EditorGUIUtility.fieldWidth = 20;
                bool customPath = EditorGUILayout.Toggle("Custom", !m_UserData.m_UseDefaultPath);
                if (customPath != !m_UserData.m_UseDefaultPath)
                {
                    m_UserData.m_UseDefaultPath = !customPath;
                    if (m_UserData.m_UseDefaultPath)
                    {
                        ResetPathToDefault();
                    }
...
//                m_UserData.m_UseDefaultPath = !isCustomOutputPath;
...
//                    m_OutputPath = EditorUserBuildSettings.GetPlatformSettings(EditorUserBuildSettings.activeBuildTarget.ToString(), "AssetBundleOutputPath");
                ...
                EditorGUILayout.BeginHorizontal();
                newState = GUILayout.Toggle(m_GenerateClass.state, m_GenerateClass.content);
                if (newState != m_GenerateClass.state)
                {
                    if (newState)
                    {
                        m_UserData.m_OnToggles.Add(m_GenerateClass.content.text);
                    }
                    else
                    {
                        m_UserData.m_OnToggles.Remove(m_GenerateClass.content.text);
                    }

                    m_GenerateClass.state = newState;
                }

                if (m_GenerateClass.state)
                {
                    var newClassPath = EditorGUILayout.TextField("Output Path", m_UserData.m_ClassPath);
                    if (newClassPath != m_UserData.m_ClassPath)
                    {
                        m_UserData.m_ClassPath = newClassPath;
                    }
                }
        ...
        private void ExecuteBuild()
        {
            if (AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory) {
                ...
                if (m_ForceRebuild.state)
                {
                    ...
                    if (m_CopyToStreaming.state)
                        message += " and " + m_streamingPath+Path.DirectorySeparatorChar+m_UserData.m_OutputPath;
                    message += "?";
                    if (EditorUtility.DisplayDialog("File delete confirmation", message, "Yes", "No"))
                    {
                        try
                        {
                            ...
                            if (m_CopyToStreaming.state)
                            if (Directory.Exists(m_streamingPath+Path.DirectorySeparatorChar+m_UserData.m_OutputPath))
                                Directory.Delete(m_streamingPath+Path.DirectorySeparatorChar+m_UserData.m_OutputPath, true);
                        }
            ...
            PTResConfigGenerator.GenerateResConfig(m_UserData.m_OutputPath,m_UserData.m_resVersion,m_GenerateClass.state,m_UserData.m_ClassPath);
            
            if(m_CopyToStreaming.state)
                DirectoryCopy(m_UserData.m_OutputPath, m_streamingPath+Path.DirectorySeparatorChar+m_UserData.m_OutputPath);
        ...
        private void BrowseForFolder()
        {
            ...
            if (!string.IsNullOrEmpty(newPath))
            {
                ...
                m_UserData.m_OutputPath = newPath;
//                EditorUserBuildSettings.SetPlatformSettings(EditorUserBuildSettings.activeBuildTarget.ToString(), "AssetBundleOutputPath",  m_UserData.m_OutputPath);
            }
        }
...
//            EditorUserBuildSettings.SetPlatformSettings(EditorUserBuildSettings.activeBuildTarget.ToString(), "AssetBundleOutputPath",  m_UserData.m_OutputPath);
        ...
        internal class BuildTabData
        {
            ...
            internal string m_resVersion = "100";
            internal string m_ClassPath = "Assets/PTAssetBundle.cs";
        }
    }
