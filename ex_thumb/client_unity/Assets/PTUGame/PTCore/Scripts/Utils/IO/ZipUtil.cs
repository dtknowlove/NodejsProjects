...
namespace PTGame.Core
{
    ...
    public class ZipUtil
    {
        ...
        public static void ZipFile(string FileToZip, string ZipedFilePath, string ZipedFileName = "", int CompressionLevel = 5, int BlockSize = 2048, bool IsEncrypt = false)
        {
            ...
            //文件名称（默认同源文件名称相同）
            var ZipFileName = string.IsNullOrEmpty(ZipedFileName) ? ZipedFilePath + Path.DirectorySeparatorChar.ToString() + new FileInfo(FileToZip).Name.Substring(0, new FileInfo(FileToZip).Name.LastIndexOf('.')) + ".zip" : ZipedFilePath + Path.DirectorySeparatorChar.ToString() + ZipedFileName + ".zip";

            using (var ZipFile = System.IO.File.Create(ZipFileName))
            {
                using (var ZipStream = new ZipOutputStream(ZipFile))
                {
                    ...
                    {
                        var fileName = FileToZip.Substring(FileToZip.LastIndexOf(Path.DirectorySeparatorChar.ToString()) + 1);
        ...
        public static void ZipDirectory(string DirectoryToZip, string ZipedPath, bool IsEncrypt = true)
        {
            ...
            using (var ZipFile = System.IO.File.Create(ZipedPath))
            {
            ...
            foreach (var file in filenames)// 遍历所有的文件和目录
            {
                ...
                else // 否则直接压缩文件
                {
                    ...
                    using (var fs = File.OpenRead(file))
                    {
                        ...
                        var fileName = parentPath + file.Substring(file.LastIndexOf(Path.DirectorySeparatorChar.ToString()) + 1);
