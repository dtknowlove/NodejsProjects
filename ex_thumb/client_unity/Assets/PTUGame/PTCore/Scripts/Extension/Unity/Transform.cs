...
namespace PTGame.Core
{
	...
	public static class TransformExtension
	{
		...
		public static T LocalEulerAngles<T>(this T selfComponent, Vector3 localAngles) where T : Component
		{
			selfComponent.transform.localEulerAngles = localAngles;
			return selfComponent;
		}

		public static T LocalEulerAnglesZ<T>(this T selfComponent, float localAnglesZ) where T : Component
		{
			var localAngles = selfComponent.transform.localEulerAngles;
			localAngles.z = localAnglesZ;
			selfComponent.transform.localEulerAngles = localAngles;
			return selfComponent;
		}
		...
		public static T Position<T>(this T selfComponent, Component component) where T : Component
		{
			selfComponent.transform.position = component.transform.position;
			return selfComponent;
		}
		...
		#region CETR0010 Destroy All Child
		
		public static T DestroyAllChild<T>(this T selfComponent, bool immediate = false) where T : Component
		{
			var childCount = selfComponent.transform.childCount;
			for (int i = childCount - 1; i >= 0; i--)
			{
				selfComponent.transform.GetChild(i).DestroyGameObjGracefully(immediate);
			}
			return selfComponent;
		}

		public static GameObject DestroyAllChild(this GameObject selfGameObj, bool immediate = false)
		{
			var childCount = selfGameObj.transform.childCount;
			for (int i = childCount - 1; i >= 0; i--)
			{
				selfGameObj.transform.GetChild(i).DestroyGameObjGracefully(immediate);
			}
			return selfGameObj;
		}
