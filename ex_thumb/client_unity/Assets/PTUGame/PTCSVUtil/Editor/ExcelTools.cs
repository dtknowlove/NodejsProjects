...
public class ExcelTools : EditorWindow
{
	...
	/// 输出格式
	/// </summary>
	private static string[] formatOption=new string[]{"CSV","JSON","XML","LUA"};
	...
	private static bool generateClass = false;


//	private Object source;


	private static CSVConfigPath configPath;
	/// <summary>
	/// 显示当前窗口	
	/// </summary>
	[MenuItem("PuTaoTool/GeekUtils/ExcelTools")]
	...
	void OnEnable()
	{
		...
		configPath = CSVConfigPath.GetConfigPath ();
	}
	...
	private void DrawOptions()
	{
		...
		configPath.csvDir = EditorGUILayout.ObjectField("导出路径",configPath.csvDir, typeof(Object), true);
		GUILayout.EndHorizontal ();

		GUILayout.BeginHorizontal ();
		generateClass = GUILayout.Toggle (generateClass,"生成对应类文件");

		if(generateClass){
			
			configPath.classDir = EditorGUILayout.ObjectField("类文件路径",configPath.classDir, typeof(Object), true);
		}
		GUILayout.EndHorizontal ();
	}
	...
	private static void Convert()
	{
		...
		foreach(string assetsPath in excelList)
		{
			...
			string dir = AssetDatabase.GetAssetPath (configPath.csvDir);
			//判断输出类型
			...
			}else if(indexOfFormat==0){
				output=output.Replace(".xlsx",".csv");
				excel.ConvertToCSV(output,encoding);

				if(generateClass){
					string classDir = AssetDatabase.GetAssetPath (configPath.classDir);
					PTCSV2Class.GenerateClassFromCsvFile (output,classDir);
				}
...
//		instance.Close();
	...
	{
		//获取当前实例
		instance=EditorWindow.GetWindow<ExcelTools>(true,"ExcelTools",true);
		//初始化
