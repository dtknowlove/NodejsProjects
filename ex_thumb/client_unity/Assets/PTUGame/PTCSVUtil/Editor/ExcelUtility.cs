...
//		mResultSet = mExcelReader.AsDataSet ();
		...
		FileStream fs=	new FileStream(excelFile, FileMode.Open);
    ...
    public void ConvertToCSV (string CSVPath, Encoding encoding)
	{
			...
			ExcelWorksheet mSheet = package.Workbook.Worksheets[1];
			if(mSheet.Dimension.End.Row < 1 )
			{
				return;
			}

		//读取数据表行数和列数
			int rowCount = mSheet.Dimension.End.Row;
			int colCount = mSheet.Dimension.End.Column;

		// 去除末尾为空的列
		for (var i = mSheet.Dimension.End.Column; i >= mSheet.Dimension.Start.Column; i--)
		{
			var line1 = mSheet.Cells[mSheet.Dimension.Start.Row, i].Value;
			if (line1 == null || string.IsNullOrEmpty(line1.ToString().Trim()))
			{
		...
		// 去除末尾为空的行
		for (var i = mSheet.Dimension.End.Row; i >= mSheet.Dimension.Start.Row; i--)
		{
			var line1 = mSheet.Cells[i,mSheet.Dimension.Start.Column].Value;
			if (line1 == null || string.IsNullOrEmpty(line1.ToString().Trim()))
			{
		...
		//读取数据
			for (int i = mSheet.Dimension.Start.Row; i <=rowCount; i++) {
				for (int j = mSheet.Dimension.Start.Column; j <= colCount; j++) {
				//使用","分割每一个数值
//					Debug.Log(mSheet.Rows [i] [j]);
				if (j == colCount) {
						stringBuilder.Append (mSheet.Cells [i,j].Value);
				} else {
						stringBuilder.Append (mSheet.Cells [i,j].Value + "@@@\t");
				}
			}
