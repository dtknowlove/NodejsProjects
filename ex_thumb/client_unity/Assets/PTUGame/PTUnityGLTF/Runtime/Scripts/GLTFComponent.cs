...
namespace UnityGLTF
{
    ...
    public class GLTFComponent : MonoBehaviour
	{
		...
		public bool AppendPersistentDataPath = false;
		...
		{
			var dir = Application.persistentDataPath + "/ddd";
			...
			client.DownloadFile(new Uri("http://10.1.223.240:8888/test/gltf/ddd.gltf"),dir+"/ddd.gltf" );
			client.Dispose();

			WebClient client2 = new WebClient();
			client2.DownloadFile(new Uri("http://10.1.223.240:8888/test/gltf/buffer.bin"),dir+"/buffer.bin" );
			client2.Dispose();
		...
		public async Task Load()
		{
			...
			try
			{
				...
				if (UseStream)
				{
					...
					}else if (AppendPersistentDataPath)
					{
					...
					Debug.LogError(directoryPath+" -->>>>>directoryPath -->");
